...
function RoomDetails() {
    const { id } = useParams()
    const { rooms, getToken,navigate } = useContext(Appcontext);
    ...
    // Check Room is AvailLabel Or Not 
    const CheckRoomAvailability = async () => {
        setFormLoading(true);
        try {
            const { data } = await axios.post("/api/booking/check-room-Availability", {
    ...
    // Add OnSumMitHandler To Check Room Avilability And Book The Room
    ...
    if (!room) {
        return (
            <div className='flex items-center justify-center min-h-[50vh] py-28'>
    ...
    return (
        <div className='py-28 md:py-35 px-4 md:px-16 lg:px-24 xl:px-32'>
            ...
            <form onSubmit={onSubmitHandler} className='flex flex-col md:flex-row items-start md:items-center bg-white justify-between shadow-[0px_0px_20px_rgba(0,0,0,0.15)] p-6 rounded-xl mx-auto mt-16 max-x-6xl'>
                ...
                <button disabled={formLoading} className='bg-primary hover:bg-primary-dull active:scale-95 transition-all rounded-md max-md:w-full max-md:mt-6 md:px-25 py-3 md:py-4 text-base cursor-pointer text-white disabled:opacity-70 disabled:cursor-not-allowed'>{formLoading ? "Loading..." : (isAvailabel ? "Book Now" : "Check Availability")}</button>
            </form>
